...
public class Solver {
     ...
     */
    public static GameGrid importPuzzleFromFile(String fileName) throws FileNotFoundException {
     ...
     */
    public static void updatePossibilities(GameGrid grid) {
        // A conventional loop to go through cells
        for (int i = 0; i < grid.DIMENSION; i++) {
            for (int j = 0; j < grid.DIMENSION; j++) {
                GameGrid.Cell current = grid.cells[i][j];
     ...
     */
    private static boolean possibleToAdd(GameGrid grid, GameGrid.Cell cell, int possibility) {
     ...
     */
    private static boolean violatesRowRule(GameGrid grid, GameGrid.Cell cell, int possibility) {
     ...
     */
    private static boolean violatesColumnRule(GameGrid grid, GameGrid.Cell cell, int possibility) {
     ...
     */
    private static boolean violatesBlockRule(GameGrid grid, GameGrid.Cell cell, int possibility) {
     ...
     */
    public static boolean step(GameGrid grid) {
        boolean flag = false;
        // For each cell
        for (int i = 0; i < grid.DIMENSION; i++) {
            for (int j = 0; j < grid.DIMENSION; j++) {
                // We're gonna use this a lot, so better store it first
                GameGrid.Cell cell = grid.cells[i][j];
     ...
     */
    private static void removePossibilityInRow(GameGrid grid, int row, int value) {
        for (int i = 0; i < grid.DIMENSION; i++) {
     ...
     * @param value the possibility to remove
     */
    private static void removePossibilityInColumn(GameGrid grid, int column, int value) {
        for (int i = 0; i < grid.DIMENSION; i++) {
     ...
     * @param value the possibility to remove
     */
    private static void removePossibilityInBlock(GameGrid grid, int block, int value) {
     ...
     * @return
     */
    public static boolean hiddenSingleInRow(GameGrid grid, GameGrid.Cell cell, int possibility) {
        int count = 0;
        for (int i = 0; i < grid.DIMENSION; i++) {
            GameGrid.Cell toCheck = grid.cells[cell.row][i];
     ...
     * @return
     */
    public static boolean hiddenSingleInColumn(GameGrid grid, GameGrid.Cell cell, int possibility) {
        int count = 0;
        for (int i = 0; i < grid.DIMENSION; i++) {
            GameGrid.Cell toCheck = grid.cells[i][cell.column];
            // If the cell is not confirmed and contains the possibility, add one to count
            if (!toCheck.confirmed && toCheck.possibilities.contains(possibility)) {
                count++;
            }
     ...
     * @return
     */
    public static boolean hiddenSingleInBlock(GameGrid grid, GameGrid.Cell cell, int possibility) {
        int count = 0;
        // Iterate through the rows in the block
        for (int i = cell.block / grid.BASE_INDEX * grid.BASE_INDEX;
             i < cell.block / grid.BASE_INDEX * grid.BASE_INDEX + grid.BASE_INDEX; i++) {
            ...
            for (int j = cell.block % grid.BASE_INDEX * grid.BASE_INDEX;
                 j < cell.block % grid.BASE_INDEX * grid.BASE_INDEX + grid.BASE_INDEX; j++) {
                GameGrid.Cell toCheck = grid.cells[i][j];
                // If the cell is not confirmed and contains the possibility, add one to count
                if (!toCheck.confirmed && toCheck.possibilities.contains(possibility)) {
                    count++;
                }
     ...
     */
    public static void confirmCell(GameGrid grid, GameGrid.Cell cell, int value) {
        cell.value = value;
        cell.confirmed = true;
